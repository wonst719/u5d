...
static _go32_dpmi_seginfo rm_old_handler, rm_new_handler, pm_old_handler,
  pm_new_handler;
...
lock_rm_new8h (void)
{
  _go32_dpmi_lock_code (rm_new8h, (unsigned long) (lock_rm_new8h - rm_new8h));
}
...
lock_pm_new8h (void)
{
  _go32_dpmi_lock_code (pm_new8h, (unsigned long) (lock_pm_new8h - pm_new8h));
}


unsigned long
pctimer_get_ticks (void)
{
  return ticks_8h;
}
...
unsigned long
pctimer_time (unsigned long start, unsigned long finish)
{
...
void
pctimer_sleep (unsigned int delayms)
{
...
void
pctimer_sound (int freq, int duration)
{
  int byte;
  unsigned int freq1;
